#!/usr/bin/env python3
"""
Optuna-based hyperparameter search for the FX offline actor-critic.

This script wraps `forex_offline_actor_critic` and uses Optuna to search over:
- Policy/value MLP architectures
- Actor noise (sigma)
- Entropy and value loss coefficients
- Reward scale
- Max units and discount factor (gamma)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Optuna search for FX offline actor-critic hyperparameters.")
    ap.add_argument(
        "--symbols",
        type=str,
    ...
    ap.add_argument("--epochs", type=int, default=8, help="Training epochs per trial (default: 8).")
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Optuna storage URL (e.g. sqlite:///forex_ac_optuna.db). Default: in-memory.",
    )
    ap.add_argument(
        "--direction",
        type=str,
    ...
    ap.add_argument(
        "--best-out",
        type=str,
        default="forex_optuna_best.json",
        help=(
            "Path to JSON file where the best trial's params/attrs will be "
            "saved whenever a new best trial is found (default: forex_optuna_best.json)."
        ),
    )
    ap.add_argument(
        "--use-tcn",
    ...
    if not symbols:
        raise SystemExit("No symbols provided for FX Optuna search.")
    ...
    X, R, dates = build_forex_dataset(base_cfg)
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        max_units = trial.suggest_float("max_units", 10.0, 500.0)
