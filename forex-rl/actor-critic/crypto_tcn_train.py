#!/usr/bin/env python3
"""
Train a shared TCN on USD-quoted Alpaca crypto pairs to predict next-day returns.

This script builds the same OHLCV-derived daily feature set as
`crypto_offline_actor_critic`, then trains a `TCNScalarHead` (from the WFO
TCN stack) across all symbols to predict the next-day log return per symbol.

The trained TCN is saved to a checkpoint that `crypto_offline_actor_critic`
...
Usage (from forex-rl root):
  ...
  python -m forex-rl.actor-critic.crypto_tcn_train \\
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train TCN for crypto next-day return prediction.")
    ap.add_argument(
        "--symbols",
        type=str,
    ...
    n = len(X)
    # Log effective aligned window after symbol/date intersection.
    ...
    if not os.path.isabs(ckpt_path):
        # Resolve relative to project root (two levels up: .../rl-trader)
