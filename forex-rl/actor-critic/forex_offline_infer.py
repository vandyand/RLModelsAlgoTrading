#!/usr/bin/env python3
"""
Inference script: produce today's raw actor outputs for OANDA FX instruments.

- Loads checkpoint from `forex_offline_actor_critic` trainer.
- Rebuilds features over a recent lookback window (daily FX OHLCV + cyclical time).
...
- Outputs a JSON mapping {instrument: action} where:
    action_i ∈ (-1, 1)
  is exactly the `tanh(mu)` output of the policy head for that instrument
  (no additional scaling or rounding).

These raw actions can then be mapped to concrete position sizes (integer OANDA
units) by a downstream aligner script, preserving the model's original semantics.
"""
...
from forex_offline_actor_critic import (  # type: ignore
    Config,
    build_forex_dataset,
    standardize_apply,
    ActorCriticMulti,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Infer today's raw actor outputs for OANDA FX instruments."
    )
    parser.add_argument(
        "--checkpoint",
        default="forex-rl/actor-critic/checkpoints/forex_offline_ac.pt",
        help="Path to FX AC checkpoint (.pt file) produced by forex_offline_actor_critic.",
    )
    parser.add_argument(
        "--environment",
    ...
    parser.add_argument(
        ...
        help="OANDA account id (used only for metadata).",
    )
    parser.add_argument(
        "--access-token",
    ...
    parser.add_argument(
        ...
        type=int,
        default=400,
        help="Number of calendar days to look back when building features (default: 400).",
    )
    parser.add_argument(
        "--use-tcn",
        action="store_true",
        help=(
            "When set, load a pre-trained FX TCN checkpoint and append its features "
    ...
    if not symbols:
        raise SystemExit("No FX symbols found in checkpoint config; cannot build universe.")

    now_utc = datetime.now(timezone.utc)
    # For daily bars, use yesterday as last complete day.
    ...
    cfg = Config(
        ...
        max_units=float(saved_cfg.get("max_units", 100.0)),
