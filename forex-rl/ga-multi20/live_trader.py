...
def submit_order(api, account_id: str, instrument: str, delta_units: int, environment: str, *, ipc_client: Optional["broker_ipc.BrokerIPCClient"] = None, broker_account_id: Optional[int] = None, cur_slip_bps: Optional[float] = None, sim_fee_perc: float = 0.0, sim_fee_fixed: float = 0.0) -> None:
    if delta_units == 0:
        return
    try:
        if api is not None:
...
def main() -> None:
    ...
    while True:
        ...
        all_pos = fetch_open_positions(api, account_id, ipc_client=ipc_client, broker_account_id=args.broker_account_id) if (api or ipc_client) else {}
        ...
        if stale_bar:
            ...
            for inst, delta in send_orders:
                if api:
                    ...
                    submit_order(None, account_id, inst, delta, args.environment, ipc_client=ipc_client, broker_account_id=args.broker_account_id, cur_slip_bps=float(cur_slip_bps), sim_fee_perc=float(args.sim_fee_perc), sim_fee_fixed=float(args.sim_fee_fixed))
                else:
