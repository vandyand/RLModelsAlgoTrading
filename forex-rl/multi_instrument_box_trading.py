        ...
        try:
            for msg in api.request(stream_request):
                        ...
                        if ckpt:
                            try:
                                ...
                                if isinstance(loaded_weights, list):
                                    ...
                                    if arr.ndim == 2 and arr.shape[1] == features.shape[0] and arr.shape[0] == 5:
                ...
                if st.open and st.current_units == 0 and st.entry_nav is not None and nav_estimate > 0:
                    ...
                    # Update policy with entry logits
                    if st.entry_features is not None and st.entry_logits is not None and st.policy is not None:
                        st.policy.update(st.entry_features, st.entry_logits, shaped_reward,
                                         args.min_tp_pips, args.max_tp_pips,
                                         args.min_sl_pips, args.max_sl_pips,
                                         args.min_trade_sec, args.max_trade_sec)
