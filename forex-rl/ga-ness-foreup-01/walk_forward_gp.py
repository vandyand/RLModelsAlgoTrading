...
def run_gp_once(
    ...
    # Evaluate on validation for best and for the entire final population
    ...
    if X_val is not None and len(X_val) > 0:
        ...
        for rank, (fit_val, ind, train_m, train_tr) in enumerate(eval_scores, start=1):
            ...
            # Compute train/val extras and size/depth for this individual
            # Positions on train for extras
            ...
            # Normalized trade frequency scores (per 1000 bars)
...
def parse_cli() -> argparse.Namespace:
    ...
    # Strategy type
    p.add_argument("--strategy", default="gp", choices=["gp", "nn", "diff"], help="Strategy representation: gp trees, nn supervised, or differentiable")
    # Differentiable strategy knobs
    p.add_argument("--diff-epochs", type=int, default=80)
    p.add_argument("--diff-lr", type=float, default=1e-3)
    p.add_argument("--diff-w-sharpe", type=float, default=1.0)
    p.add_argument("--diff-w-cum", type=float, default=0.5)
    p.add_argument("--diff-w-val-sharpe", type=float, default=1.0)
    p.add_argument("--diff-w-gap", type=float, default=0.3)
    p.add_argument("--diff-w-ratio", type=float, default=0.3)
    p.add_argument("--diff-w-tf-band", type=float, default=0.1)
    p.add_argument("--diff-w-exposure", type=float, default=0.1)
    p.add_argument("--diff-exposure-target", type=float, default=0.2)
    p.add_argument("--diff-tf-low", type=float, default=10.0)
    p.add_argument("--diff-tf-high", type=float, default=25.0)
    p.add_argument("--diff-tf-maxok", type=float, default=40.0)
        ...
        for i, (ts_train_s, ts_train_e, ts_val_s, ts_val_e) in enumerate(windows):
            ...
            try:
                if a.strategy == "nn":
                    ...
                    from nn_strategy import NNConfig, run_nn_once
                    # Train and evaluate neural strategy per window (no GP inner loop)
                    nn_cfg = NNConfig()
                    result = run_nn_once(
                        X_train,
                        X_val,
                        close_col,
                        nn_cfg,
                        min_hold=int(a.min_hold),
                        cooldown=int(a.cooldown),
                        seed=window_seed,
                    )
                    cfg.score_fn = None  # not used in this branch
                elif a.strategy == "diff":
                    # Lazy import to avoid hard dependency when unused
                    from differentiable_strategy import DiffConfig, run_diff_once
                    diff_cfg = DiffConfig(
                        epochs=int(a.diff_epochs),
                        lr=float(a.diff_lr),
                        w_sharpe=float(a.diff_w_sharpe),
                        w_cum=float(a.diff_w_cum),
                        w_val_sharpe=float(a.diff_w_val_sharpe),
                        w_gap=float(a.diff_w_gap),
                        w_ratio=float(a.diff_w_ratio),
                        w_tf_band=float(a.diff_w_tf_band),
                        w_exposure=float(a.diff_w_exposure),
                        exposure_target=float(a.diff_exposure_target),
                        tf_low=float(a.diff_tf_low),
                        tf_high=float(a.diff_tf_high),
                        tf_maxok=float(a.diff_tf_maxok),
                    )
                    result = run_diff_once(
                        X_train,
                        X_val,
                        close_col,
                        diff_cfg,
                    ...
                    # Optional: build/update learned fitness function from past windows (GP only)
            ...
            rec = {
                ...
                "time_sec": dt,
                "best_tree": result.get("best_tree"),
                "train_metrics": tm,
                "val_metrics": vm,
                "deg": {"sharpe": deg_sh, "cumret": deg_cr},
                ...
                "scaled": {
                    "train": scaled_fields(tm),
                    "val": scaled_fields(vm) if vm else None,
            ...
            print(json.dumps({
                "event": "wfo_window",
                "i": i,
                "train": {"shrp": train_sh, "cum": train_cr},
                "val": {"shrp": val_sh, "cum": val_cr},
                "deg": {"shrp": deg_sh, "cum": deg_cr},
                "sec": dt,
