#!/usr/bin/env python3
"""
PCA reducer for grid features.

- Loads wide features CSV produced by grid_features.py
- Standardizes each column using train split means/stds
- Fits PCA to retain N components (e.g., 50) that explain most variance
- Saves transformed features and PCA artifacts (component matrix, mean/std, explained variance)

Usage:
  python forex-rl/supervised-ae/pca_reduce.py \
    --features forex-rl/supervised-ae/data/multi_features.csv \
    --components 50 \
    --train-ratio 0.8 \
    --out-features forex-rl/supervised-ae/data/pca_features.npy \
    --out-meta forex-rl/supervised-ae/data/pca_meta.npz
"""
...
def main() -> None:
    ...
    parser.add_argument("--out-features", default="forex-rl/supervised-ae/data/pca_features.npy")
    parser.add_argument("--out-meta", default="forex-rl/supervised-ae/data/pca_meta.npz")
    ...
    Z = np.vstack([Z_train, Z_val]).astype(np.float32)
