...
def summarize_windows(windows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "deg": {
            "sharpe_mean": float(np.nanmean(np.array(val_sh) / np.array(train_sh))) if win_count > 0 else float("nan"),
            "cumret_mean": float(np.nanmean(np.array(val_cr) / np.array(train_cr))) if win_count > 0 else float("nan"),
        },
        "means": {
