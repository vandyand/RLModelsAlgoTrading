...
def build_and_save(args: Args) -> None:
    ...
    if etf_list:
        print({"status": "fetch_etf_batch", "tickers": etf_list[:5], "count": len(etf_list)})
        etf_map = fetch_etf_ohlcv(etf_list, args.start, args.end)
        for tkr, df in etf_map.items():
            print({"status": "compute_etf_features", "ticker": tkr})
    ...
    # Save date index as naive YYYY-MM-DD
    if isinstance(base_index, pd.DatetimeIndex):
        if base_index.tz is not None:
            idx_naive = base_index.tz_convert('UTC').tz_localize(None)
        else:
            idx_naive = base_index
    else:
        idx_naive = pd.to_datetime(base_index)
    pd.Series(idx_naive).dt.strftime('%Y-%m-%d').to_csv(args.out_dates, index=False, header=False)
...
def parse_cli() -> Args:
    ...
    parser.add_argument("--out-features", default="forex-rl/unsupervised-ae/data/multi_features.csv")
    parser.add_argument("--out-returns", default="forex-rl/unsupervised-ae/data/fx_returns.csv")
    parser.add_argument("--out-dates", default="forex-rl/unsupervised-ae/data/dates.csv")
    a = parser.parse_args()
...
if __name__ == "__main__":
    ...
    print({
        ...
        "use_all_etfs": bool(args.etf_tickers is None and args.use_all_etfs),
        "etf_override": bool(args.etf_tickers),
        "start": args.start,
        "end": args.end,
    })
    try:
